...
filter._filterMatches = (filterElement, attributeValue) => {
  ...
  const filterFunction = {
    ...
    '!:': function filterCaseInsensitiveExcludes (attrValue, filterValue) {
      return attrValue.toLowerCase().indexOf(filterValue.toLowerCase()) === -1
    }
  }[filterElement.operator]
  const result = filterFunction ? filterFunction(attributeValue, filterElement.value) : false;
  return result
}
