...
filter._splitElement = element => {
  ...
  let match = FILTER_OPERATORS.indexOf(element[0]) !== -1;
  if (match > -1) {
    let length = FILTER_OPERATORS[match].length;
    return { operator: element.substr(0, length), value: element.substr(length) }
  }
